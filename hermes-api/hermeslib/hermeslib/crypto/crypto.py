...
""" int: encryption and decryption block sizes
    ...
    The encryption block size refers to tbe maximum size of a string that the 
    rsa library will accept as input. Consequently the decryption block size is 
    the size of the output that an input of size 501 will produce.
"""
...
def gen_random(num_bytes):
    """ Generates some number of secure random bytes based on the parameter that
        specifies how many bytes to generate. This function is secure enough to
        generate secure keys for AES.

        Args:
            num_bytes (int): The number of bytes to generate and return.

        Returns:
            bytearray: An array of randomly generated bytes whose size is equal
                       to the num_bytes value.
    """
    return Random.get_random_bytes(num_bytes)


def gen_symmetric_key():
    """ This function generates a key for symmetric encryption. More 
        specifically it uses a secure randomness generator from the PyCrypto 
        library (wrapper defined above) to generate 32 random bits to use as a 
        key for an AES256 symmetric encryption cipher.

        Args: None

        Returns:
            bytearray: A byte array of size 32 containing secure, randomly 
                       generated bits that can be used as a key for an AES256 
                       cipher.
    """
...
def symmetric_encrypt(msg, key):
    """ Encrypts a given message with a symmetric cipher, AES256 to be specific,
        and returns the ciphertext as an array of bytes. The ciphertext actually
        begins with an IV which is a random sequence of numbers that are used to
        initialize the cipher, but so long as you use the symmetric_decrypt 
        function included below on the ciphertext you should get the plaintext 
        back without the IV.
    
    Args:
        msg (bytearray): The message that is to be encrypted.
        key (bytearray): The key, length 32, with which the message is to be 
                         encrypted.
...
def symmetric_decrypt(ciphertext, key):
    """ Decrypts a ciphertext that was encrypted with a symmetric cipher, 
        AES256 to be specific, and returns the plaintext as an array of bytes. 
        The key must be the same key that the message was originally encrypted 
        with. This function, along with its counterpart, symmetric_encrypt, is 
        used to encrypt messages inside of sessions, but it can also be used to 
        encrypt/decrypt information that needs to be stored on the hard-drive. 
        Just be sure to read the file as bytes by using the "rb" flag for the 
        open() function. This function does not take strings.

        If you use the symmetric_encrypt function, the result is actually the 
        ciphertext with an IV value prepended. This value is used to initialize 
        the cipher before the actual decryption. Therefore, there is a set of 
        bytes at the start of the bytearray that are completely random. The 
        number of these bytes is equal to the block size. The function uses 
        these bytes to initialize the cipher object and then decrypt the 
        message.
    ...
    Returns:
        bytearray: A bytearray containing the plaintext of the message that was 
                   decrypted.
...
def sha256(message):
    """ Returns the SHA256 hash of a given message. In hermes, this function is
        used most often to generate IDs of users by hashing their public key.
        This allows us to generate an ID based on a public key that is much
        easier to handle than a public key.

        Args:
            message (bytearray): The message to be hashed, as a byte array.

        Returns:
            bytearray: The bits that result from hashing the message.
    """
...
def asymmetric_decrypt_verify(ciphertext, decrypt_key, verify_key):
    """ This function decrypts a message that was encrypted with an asymmetric 
        cipher. In this case, we use an RSA algorithm. This function returns the
        plaintext of the message from the ciphertext only if the message 
        signature is valid. Specifically, this function expects the signature to
        be the first 512 bytes of the decrypted message, so once it acquires the
        plaintext, it extracts the signature. This function expects the 
        ciphertext along with a key to be used for decryption (the recipient's 
        public key) and the verification key (the sender's public key). It 
        decrypts the ciphertext and then extracts the signature and verifies it.
        If the signature is verified then the function returns the decrypted 
        message, otherwise it returns None and logs a warning that the 
        verification failed. This function also returns None and logs when there
        is a decryption failure or a type error (when the wrong type is given in
        place of the ciphertext).

    Args:
        ciphertext (bytearray): The ciphertext that is to be decrypted. For best
                                results, this should be the ciphertext that was 
                                produced by the asymmetric_encrypt_sign() 
                                function. The ciphertext should be the encrypted
                                message with its signature of 512 bytes 
                                prepended to it before encryption.
        decrypt_key (rsa.PrivateKey): The private key object that should be used
                                      to decrypt the message.
        verify_key (rsa.PublicKey): The public key object that should be used to
                                    verify the signature of the message.
    Returns:
        bytearray: The ciphertext if the message can be decrypted and verified,
                   and None otherwise as well as logging certain common types of
                   errors.
...
def asymmetric_verify(signature, plaintext, verify_key):
    """ This function verifies the signature (bytearray) of a message given the 
        original plaintext (bytearray) and the public key with which to verify 
        the signature (rsa.PublicKey). The function returns true if the 
        signature is valid or false if the signature is invalid or if the type 
    ...
    Args:
        ...
        verify_key (rsa.PublicKey): The private key object that should be used 
...
def asymmetric_decrypt(ciphertext, decrypt_key):
    """ This function decrypts a message that was encrypted with an asymmetric 
        cipher. In this case we use an RSA algorithm. This function returns the 
        plaintext of the message from the ciphertext even if the message 
        signature is invalid. The function expects the first 512 bytes of the 
        decrypted plaintext to be the signature of the message. Once the 
        ciphertext is decrypted it separates the signature from the plaintext 
        and returns both. If the decryption fails the function returns (None, 
        None), otherwise the function returns the plaintext and the signature 
        regardless of whether the signature is valid.

        It is not advisable to use this function for anything other than 
        debugging with the exception of the function 
        asymmetric_decrypt_verify_public_key() which uses it to decrypt the 
        message and then verify the signature with the public key contained in 
        the message.
    ...
    Returns:
        bytearray: The plaintext of the decrypted message or none if the
                   message cannot be decrypted.
        bytearray: The signature of the decrypted message or none if the
                   message cannot be decrypted. Please note that a returned
                   signature does not imply that the signature is valid as this
                   function does not check for validity. If you require to check
                   for signature validity you should be using the
...
def asymmetric_encrypt_sign(plaintext, encrypt_key, sign_key):
    """ This function is identical to the one above except that it only returns 
        the ciphertext of the signature and message and not the plaintext 
        signature. This is the function you should probably be using.
